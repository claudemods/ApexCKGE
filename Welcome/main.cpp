...
public:
    MainWindow(QWidget *parent = nullptr) : QWidget(parent) {
        ...
        setWindowTitle("Apex CKGE Full v1.02");
        ...
        gifTextLabel->setText(QString(
            ...
            "Apex CachyOS Kde Gamer's Edition Minimal v1.02"
            "</span>"
            "</div>"
        ));
        ...
        QPushButton *apexInstallerButton = new QPushButton("Install With Apex Arch Installer Gui Ext4", this);
        ...
        // New Btrfs Installer button
        QPushButton *apexBtrfsInstallerButton = new QPushButton("Install With Apex Arch Installer Btrfs", this);
        apexBtrfsInstallerButton->setStyleSheet("background-color: #00568f; color: #24ffff; font-size: 16px;");
        connect(apexBtrfsInstallerButton, &QPushButton::clicked, this, &MainWindow::runApexBtrfsInstaller);
        layout->addWidget(apexBtrfsInstallerButton);
...
private slots:
    ...
    void runApexInstaller() {
        ...
        process->start("/usr/bin/ApexArchInstaller-v1.01.AppImage");
    }

    void runApexBtrfsInstaller() {
        QProcess *process = new QProcess(this);
        connect(process, QOverload<QProcess::ProcessError>::of(&QProcess::errorOccurred), this, [this, process](QProcess::ProcessError error) {
            qDebug() << "Error occurred:" << error;
            QMessageBox::warning(this, "Error", "Failed to start Apex Btrfs Installer: " + process->errorString());
            process->deleteLater();
        });

        connect(process, QOverload<int, QProcess::ExitStatus>::of(&QProcess::finished), this, [this, process](int exitCode, QProcess::ExitStatus exitStatus) {
            qDebug() << "Apex Btrfs Installer finished with exit code:" << exitCode;
            process->deleteLater();
        });
    ...
    void downloadAndShowChangeLog() {
        ...
        QUrl url("https://github.com/claudemods/ApexCKGE/blob/main/Full%20Stable%20And%20Development.md");
